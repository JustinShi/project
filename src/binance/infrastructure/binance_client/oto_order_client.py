...
from binance.config.constants import OrderSide, OrderType
...
class BinanceOTOOrderClient:
    ...
    def _resolve_alpha_symbol(self, symbol: str, chain: Optional[str] = None) -> ResolvedSymbol:
    ...
    @staticmethod
    def _extract_tick_size(price_filter: Optional[Dict[str, str]]) -> Optional[Decimal]:
        if not price_filter:
            return None
    ...
    async def get_order_status(self, symbol: str, order_id: str) -> Tuple[bool, str, Optional[Dict]]:
        """查询订单状态
    ...
    async def get_open_orders(self, symbol: Optional[str] = None) -> Tuple[bool, str, Optional[list]]:
        """获取未成交订单列表"""
        try:
            await self._ensure_client()
