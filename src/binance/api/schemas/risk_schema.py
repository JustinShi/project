...
class RiskProfileCreateRequest(BaseModel):
    ...
    weekend_trading: bool | None = Field(None, description="是否允许周末交易")
    max_consecutive_losses: int | None = Field(None, description="最大连续亏损次数")
    max_daily_loss: Decimal | None = Field(None, description="每日最大亏损")
...
class RiskAlertResponse(BaseModel):
    ...
    risk_level: str = Field(..., description="风险等级")
    current_value: Decimal | None = Field(None, description="当前值")
    ...
    duration_seconds: int | None = Field(None, description="持续时间（秒）")
...
class RiskMetricsResponse(BaseModel):
    ...
    last_order_time: datetime | None = Field(None, description="最后订单时间")
