...
async def get_user_notifications(
    ...
    try:
        # 获取通知历史
        notifications_data = await notification_service.get_notification_history(
            user_id, limit
        )
...
async def mark_notification_read(
    ...
    try:
        success = await notification_service.mark_notification_read(
            user_id=user_id, notification_id=request.notification_id
        )
        ...
        if success:
            ...
            return NotificationMarkReadResponse(
                success=False, message="标记通知已读失败"
            )
...
async def get_notification_statistics(
    ...
    try:
        ...
        unread_notifications = sum(
            1 for n in notifications_data if not n.get("is_read", False)
        )
        ...
        for notif in notifications_data:
            ...
            notifications_by_type[notif_type] = (
                notifications_by_type.get(notif_type, 0) + 1
            )
...
async def update_notification_settings(
    ...
    try:
        ...
        return NotificationSettingsResponse(
            ...
            volatility_alerts=request.volatility_alerts
            if request.volatility_alerts is not None
            else True,
            order_updates=request.order_updates
            if request.order_updates is not None
            else True,
            balance_alerts=request.balance_alerts
            if request.balance_alerts is not None
            else True,
        )
