...
from binance.infrastructure.config import YAMLConfigManager, TradingTarget
...
async def get_order_monitor(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取订单监控信息失败: {str(e)}")
...
async def cleanup_timeout_orders(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"清理超时订单失败: {str(e)}")
